...
void MainWindow::on_enlazar_clicked()
{
    ...
    for (Neurona &neurona : lista) {
        ...
        neurona.agregar_arista(&neurona);
    }
}
...
void MainWindow::on_profundidad_clicked() {
    list<Neurona>& lista = listaNeuronas.listaNeurona();
    QString idInicio = ui->idProfundidad->toPlainText();
    int idInicioInt = idInicio.toInt();
    ui->profundidadList->clear();

    Neurona* nodoInicial = nullptr;
    for (Neurona& nodo : lista) {
        if (nodo.getId() == idInicioInt) {
            nodoInicial = &nodo;
            break;
        }
    }
    ...
    if (!nodoInicial) {
        ui->profundidadList->insertPlainText("No se encontro ese ID");
        return;
    }

    int x = 0, y = 0;
    FuerzaBrutaBuscarCercania(x, y, lista, *nodoInicial);

    Neurona* nodoCercano = nullptr;
    for (Neurona& nodo : lista) {
    ...
    if (!nodoCercano) {
        ui->profundidadList->insertPlainText("No hay Aristas Conectadas a este Nodo");
        return;
    }

    nodoInicial->agregar_arista(&(*nodoCercano));
    for (Neurona& nodo : lista) {
        if (&nodo != nodoInicial) {
            int xNeighbor = 0, yNeighbor = 0;
            FuerzaBrutaBuscarCercania(xNeighbor, yNeighbor, lista, nodo);
            Neurona* neighbor = nullptr;
            for (Neurona& n : lista) {
                if (n.getPosX() == xNeighbor && n.getPosY() == yNeighbor) {
                    neighbor = &n;
                    break;
                }
            }
            if (neighbor) {
                nodo.agregar_arista(neighbor);
            }
        }
    }
    listaNeuronas.busquedaProfundidad(*nodoInicial, ui->profundidadList);
}
